...
`/create-spec`, `/plan-feature`, `/implement-feature`, `/verify-ui`, `/review`, `/feature-report`, `/update-docs`, `/new-module`, `/encode-lesson` — each is defined in `.claude/skills/` and explains itself. Reviewer personas live in `.claude/agents/` and judge against `docs/personas/`.
